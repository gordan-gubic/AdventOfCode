...
public class Day25_02 : Day
{
    private const int YEAR = 2023;
    private const int DAY = 25_02;
    ...
    public Day25_02(ILog log, IParser parser) : base(log, parser, YEAR, DAY)
    {
        EnableDebug();
        Initialize();
    ...
    private HashSet<string> FindGroups((string, string) rel1, (string, string) rel2, (string, string) rel3)
    {
        var links = _links.CloneDeep();

        RemoveLink(links, rel1);
        RemoveLink(links, rel2);
        RemoveLink(links, rel3);
        ...
        while (queue.Count > 0)
        {
            ...
            ProcessItem(item, links, group, queue);
        }

        return group;
    }

    private void ProcessItem(string item, Dictionary<string, HashSet<string>> links, HashSet<string> group, Queue<string> queue)
    {
        group.Add(item);
        links[item].ForEach(queue.Enqueue);
    }
