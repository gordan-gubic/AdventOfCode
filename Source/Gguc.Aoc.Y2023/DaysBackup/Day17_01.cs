...
public class Day17_01 : Day
{
    private const int YEAR = 2023;
    private const int DAY = 17_01;

    private Map<int> _map;
    // private HashSet<(int, int, long, char, int)> _cache;
    private Dictionary<(int, int, char, int), long> _cache;

    public Day17_01(ILog log, IParser parser) : base(log, parser, YEAR, DAY)
    {
        EnableDebug();
        Initialize();
    ...
    private long FindPath(Point start, Point end)
    {
        ...
        var init = new Crucible { Point = start, Sum = 0L, Dir = 'E', Count = 0 };
